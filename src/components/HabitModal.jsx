...
export default function HabitModal({ open, onClose, onSave, categories }) {
  ...
  async function handleSubmit(event) {
    ...
    const saved = await onSave({
      ...
      target: Math.max(1, Number(form.target)),
      unit: form.unit.trim() || 'veces',
  ...
  return (
    <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <form className="editor-modal panel enter-up" onSubmit={handleSubmit}>
        <div className="modal-heading">
          <div><p className="eyebrow">CONSTANCIA</p><h2>Nuevo hábito</h2></div>
        ...
        <div className="form-grid two-columns">
          ...
          <label><span>Meta diaria</span><input type="number" min="1" value={form.target} onChange={(event) => updateField('target', event.target.value)} /></label>
          ...
          <label><span>XP base por día</span><input type="number" min="1" max="100" value={form.xp_reward} onChange={(event) => updateField('xp_reward', event.target.value)} /></label>
        </div>
