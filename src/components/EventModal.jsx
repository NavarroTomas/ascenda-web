...
export default function EventModal({ open, event, defaultDate, onClose, onSave }) {
  ...
  useEffect(() => { if (open) setForm({ ...EMPTY, ...(defaultDate ? { event_date: defaultDate } : {}), ...(event || {}), start_time: event?.start_time?.slice(0,5) || '', end_time: event?.end_time?.slice(0,5) || '', recurrence_until: event?.recurrence_until || '' }) }, [open, event, defaultDate])
  if (!open) return null
  const update = (key, value) => setForm((current) => ({ ...current, [key]: value }))
  async function submit(e) { e.preventDefault(); setSaving(true); const ok = await onSave({ ...form, title: form.title.trim(), description: form.description.trim() || null, start_time: form.all_day ? null : form.start_time || null, end_time: form.all_day ? null : form.end_time || null, location: form.location.trim() || null, link_url: form.link_url.trim() || null, guests_text: form.guests_text.trim() || null, notes: form.notes.trim() || null, recurrence_until: form.recurrence_type === 'none' ? null : form.recurrence_until || null }, event); setSaving(false); if (ok) onClose() }
  return <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><form className="editor-modal panel enter-up" onSubmit={submit}>
    ...
    <div className="form-grid two-columns">
      <label><span>Fecha</span><input type="date" required value={form.event_date} onChange={(e) => update('event_date', e.target.value)} /></label>
      <label className="setting-row compact-setting"><span><strong>Todo el día</strong></span><input type="checkbox" checked={form.all_day} onChange={(e) => update('all_day', e.target.checked)} /></label>
      {!form.all_day && <><label><span>Inicio</span><input type="time" value={form.start_time} onChange={(e) => update('start_time', e.target.value)} /></label><label><span>Fin</span><input type="time" value={form.end_time} onChange={(e) => update('end_time', e.target.value)} /></label></>}
      ...
      {form.recurrence_type !== 'none' && <label><span>Repetir hasta</span><input type="date" value={form.recurrence_until || ''} onChange={(e) => update('recurrence_until', e.target.value)} /></label>}
      <label><span>Estado</span><select value={form.status} onChange={(e) => update('status', e.target.value)}><option value="confirmed">Confirmado</option><option value="tentative">Tentativo</option><option value="cancelled">Cancelado</option></select></label>
      <label><span>Color</span><input type="color" value={form.color} onChange={(e) => update('color', e.target.value)} /></label>
      <label><span>Ubicación</span><input value={form.location || ''} onChange={(e) => update('location', e.target.value)} /></label>
      <label><span>Enlace</span><input value={form.link_url || ''} onChange={(e) => update('link_url', e.target.value)} placeholder="https://…" /></label>
    </div>
